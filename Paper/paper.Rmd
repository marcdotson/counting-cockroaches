...
To use our model, we fed a set of tweets into a PCA model. The output of the PCA model (unsupervised) was then fed into an XGBoost model (supervised). Simultaneously, we fed the same set of tweets into a fastText classification model (supervised). The frankenstien model then compares predictions of the XGBoost and fastText models, and makes a final prediction. I believe that the idea behind this "frankenstein model" is to use the perspectives of various styles of models to gain an accurate consensus on the data -- somewhat similar to the concept behind random forests.
