...
This is a simplified version of the data generating formula that creates
a data frame with only three variables instead of five. For simplicity
sake income and state have been excluded.

``` r
simulate_mrp_data <- function(n) {
  J <- c(2, 2, 2) # male or not, eth, age, income level, state
  poststrat <- as.data.frame(array(NA, c(prod(J), length(J)+1))) 
  # Columns of post-strat matrix, plus one for size
  colnames(poststrat) <- c("male", "eth", "age")
  count <- 0
  for (i1 in 1:J[1]){
    for (i2 in 1:J[2]){
      for (i3 in 1:J[3]){
            count <- count + 1
            # Fill them in so we know what category we are referring to
            poststrat[count, 1:3] <- c(i1-1, i2, i3)
          }
        }
      }
  # Proportion in each sample in the population
  ...
  coef_age_male <- t(cbind(c(0, .1, .23, .3, .43, .5, .6),
                           c(0, -.1, -.23, -.5, -.43, -.5, -.6)))
  true_popn <- data.frame(poststrat[, 1:3], service_failure = rep(NA, prod(J)))
  for (j in 1:prod(J)) {
    true_popn$satisfaction[j] <- plogis(
      coef_male[poststrat[j, 1] + 1] +
        coef_eth[poststrat[j, 2]] + coef_age[poststrat[j, 3]] +
...
data {
  ...
  int<lower=1,upper=L> ll[N];    /p / Group assignment. 
...
The data section of the Downes model not only specifies the individual
predictor values but also the number of categories for each categorical
variable. Each category is given a varying coefficient in the parameter
section.
...
parameters {                                          // Model parameters to be estimated:
  ...
  real a_eth[n_eth];                                  // ethnicity class varying coeffs 
  real a_income[n_income];                            // income varying coeffs (depart linear fit)
  real a_male[n_male];                                // gender varying coeffs
  real a_state[n_state];                              // state varying coeffs
  real<lower=0> sigma_age;                            // age var(sd) component
  real<lower=0> sigma_eth;                            // ethnicit var(sd) component
  real<lower=0> sigma_income;                         // income var(sd) component
...
```
    ...
    ## Warning in max(sample$income): no non-missing arguments to max; returning -Inf

    ## Warning in max(sample$state): no non-missing arguments to max; returning -Inf
...
The data for the Stan User Guide model is specified with lower and upper
bounds on the data for the number of categories for each variable. In
this stan model, post-stratification takes place in the generated
quantities section.

\[Note to McKenna\] Questions to answer: What the epsilon, -epsilon is
that?
