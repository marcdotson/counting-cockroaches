...
Similar to the previous model, we need to create average tweet vectors for each tweet.
```{r}
...
```{r}
test_data %>% 
  ggplot(aes(x = complaint_label, fill = predict_label)) +
  geom_bar()
```
...
save(train, file = ../../Output/train.Rdata) # Save the training file so it can be referenced in the paper
```
