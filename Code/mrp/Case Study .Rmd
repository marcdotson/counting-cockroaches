...
We are woking towards building a model that can predict the severity of a service failure baised on consumer feedback. The case study will work to build a simple model and impliment postratification to generalize findings in a sample to a larger population. The model we will focus on here will start as a simple binomial model but we will hopefully build it up. We will be using code taken from Lauren Kenndey and Jonah Gabry's vignette on postratification see: http://mc-stan.org/rstanarm/articles/mrp.html
...
Becasue we are using simulated data we can see how our poststratified predictions compare to the true population 
