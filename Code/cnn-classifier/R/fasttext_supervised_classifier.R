...
# Dallin Note: Throughout this script, I've adjusted file paths to make them run well on my computer
file_model <- "Temporary/train_test_data/class_mod_1.dat"
train_data <- "Temporary/train_test_data/train_data_1.txt"
test_data <- "Temporary/train_test_data/test_data_1.txt"

# read in the data
# tweet_data <- read_csv("data/Marketing Research Labeled Tweets_ - tweet_sample_5k_FULL.csv") %>% 
#   filter(nchar(tweet_text) > 5)

# Dallin Note: For my purposes, I'll edit the code directly above and adjust it for my computer
tweet_data <- read_csv("Temporary/train_test_data/Marketing Research Labeled Tweets_ - tweet_sample_5k_Ky-Ch-Ad.csv") %>%
  filter(nchar(tweet_text) > 5)
...
test_labels_without_prefix <- test_train_data %>% 
  filter(test_train == "test") %>% 
  pull(label)
...
get_hamming_loss(as.list(test_labels_without_prefix), as.list(predictions))
